...
| | |
|---|---|
| Bundle id / package | `com.altixcode.capflow` |
| App Store Connect app | `6811948162` (version 1.0, `PREPARE_FOR_SUBMISSION`) |
| In-app purchase | `com.altixcode.capflow.removeads` — ASC id `6811948768`, state `MISSING_METADATA` |
| RevenueCat project | `proj0679a1f5` |
| RevenueCat apps | iOS `appacb12abafa` · Android `app99e10f481e` |
...
- **Interstitial**: app/studio.tsx — after a captioned video is saved. Pacing lives in `adPolicy.ts`: the first completion
...
- **Tests**: 102 passing, typecheck clean.
...
2. **Write and upload the store listing.** This app has no description and no
   ...
   `Dev/scripts/store-metadata.json`; add an entry for CapFlow and upload it with
...
4. **Upload the IAP review screenshot.** The in-app purchase
   (`com.altixcode.capflow.removeads`) is localized and priced at $3.99, and sits at
   `MISSING_METADATA` for want of one screenshot:

   ```bash
   asccli iap-review-screenshot upload --iap-id 6811948768 --file <path-to-png>
   ```
...
2. **RevenueCat — Apple credentials.** `appacb12abafa` (the App Store app in project
   `proj0679a1f5`) has no Apple credentials, so App Store purchases cannot be
   validated. This needs an interactive Apple ID sign-in with 2FA:

   ```bash
   rc setup apple appacb12abafa
   ```
...
3. **Create the GitHub repo first — there is none.** `git remote -v` is empty, so
   ...
   and nothing is backed up. Create `AltixCode/capflow` and push, then add
   ...
   Set each with `gh secret set <KEY> --repo AltixCode/capflow`. The RevenueCat
   public SDK keys are fetchable — `rc api GET "/projects/proj0679a1f5/apps/appacb12abafa/public_api_keys"`
   — the AdMob ones come from step 1.

4. **The legal URLs are wrong and they 404.** `src/config/legal.ts` points at
   `https://www.hushtunnel.com/legal/capflow-privacy` — **hushtunnel.com**, a leftover from the HushTunnel
...
5. **Play Console — create the app.** `com.altixcode.capflow` is not recognised by
...
../scripts/verify-app.sh capflow com.altixcode.capflow   # full build + device verification, both platforms
```
